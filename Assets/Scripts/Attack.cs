...
public class Attack : MonoBehaviour
{
    ...
    private float _nextHardAttackTime = 0.0f;
    private float _nextFistAttackTime = 0.0f;
    [SerializeField] private Animator _animator;
    
    void Update()
    {
        if (Input.GetButtonDown("Fire1") && Input.GetKey(KeyCode.LeftShift) && !isHardAttacking && Time.time > _nextHardAttackTime)
        {
            _cooldownTime = 10f;
            HardAttack();
            isHardAttacking = true;
            _nextHardAttackTime = Time.time + _cooldownTime;
            
        }
        HardAttackAnimation();

        if (Input.GetButtonDown("Fire1") && !isHardAttacking && Time.time > _nextFistAttackTime)
        {
            _cooldownTime = 2f;
            FistAttack();
            isFistAttacking = true;
            _nextFistAttackTime = Time.time + _cooldownTime;
        }
